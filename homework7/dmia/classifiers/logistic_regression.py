...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        probabilities = LogisticRegression.logistic(X.dot(self.w))
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        h = LogisticRegression.logistic(X_batch.dot(self.w))
        dw = self.gradient(X_batch, self.w, y_batch)
        loss = np.sum(-y_batch * np.log(h) - (1.0 - y_batch) * np.log(1.0 - h))
        ...
        loss = loss + reg * np.sum(self.w[:-1] ** 2)
        dw[:-1] = dw[:-1] + reg * self.w[:-1]
        return loss, dw

    def gradient(self, x, w, t):
        return (self.logistic(x.dot(w)) - t).T * x
